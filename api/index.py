...
def handle_get_offered_courses(body, context):
    cookies = body.get("cookies", {})
    if not cookies:
        context._send_response(
            401,
    ...
    offered_service = ServiceFactory.create_offered_courses_service()
    result = offered_service.get_offered_courses(cookies=cookies)
    context._send_json_response(result)
...
def handle_get_course_registration_summary(body, context):
    cookies = body.get("cookies", {})
    if not cookies:
        context._send_response(
            401,
    ...
    summary_service = ServiceFactory.create_course_registration_summary_service()
...
def handle_get_curriculum_status(body, context):
    cookies = body.get("cookies", {})
    if not cookies:
        context._send_response(
            401,
    ...
    curriculum_service = ServiceFactory.create_curriculum_status_service()
    result = curriculum_service.get_curriculum_status(cookies=cookies)
    context._send_json_response(result)
...
dispatcher.register('get_curriculum_status', handle_get_curriculum_status)
